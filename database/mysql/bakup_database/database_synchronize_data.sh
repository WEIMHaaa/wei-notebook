...
# 2、源数据库压缩备份
bak_scr() {
  mysqldump -h127.0.0.1 -P3306 -uroot -ppassword ${src_dbname_list[$i]} | gzip >/deploy_demo/src_dbname/${time1}/${src_dbname_list[$i]}.sql.gz
...
for i in ${!src_dbname_list[@]}; do
  ...
  for j in ${!dst_dbname_list[@]}; do
    # $i -eq $j ：表示参数i和参数j的位置相同, 注：数据库位置一定要对应！！！
    if [ $i -eq $j ]; then
      sync_dst
    fi
  done
