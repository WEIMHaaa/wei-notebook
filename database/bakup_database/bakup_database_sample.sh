...
#1.1、备份单个数据库dbname到目录/tmp下面,并指定名称为dbname(dbname：数据库名称, 也可任意命名)
...
#1.3、备份数据库中的某个表t_a_table t_b_table,注：表名之间是空格
mysqldump -h127.0.0.1 -uroot -ppassword dbname t_a_table t_b_table > /tmp/dbname.sql;

#1.4、备份多个数据库a_dbname b_dbname,注：加上--databases参数,数据dbname之间是空格
...
#1.7、排除表进行备份
#--ignore-table=数据库名.表名,该参数的作用是,对某些表不进行导出。等号后面的参数即表的信息,必须通过数据库和表名来定义。如果要忽略多张表,那么就要多次使用--ignore-table参数
...
#--no-create-db,        ---取消创建数据库sql(默认存在),只导出数据,而不添加CREATE DATABASE语句
...
##############################还原##############################
#2.1、将dbname.sql还原到dbname数据库,执行前需要先创建dbname数据库
...
mysql>use dbname
mysql>source /tmp/dbname.sql  #使用source命令,后面参数为脚本文件（如这里用到的.sql）
