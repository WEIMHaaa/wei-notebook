...
# 快照版格式1：yyyyMMdd.hhmmss,举例：bsf-api-1.0.4-20201021.093600-1.jar,只检测带项目名api包含202的快照版
# 快照版格式2：SNAPSHOT,举例：bsf-api-1.0.4-SNAPSHOT-1.jar,只检测带项目名api包含SNAPSHOT的快照版
#-----------------------------------------------------------------------------

comp=${1}      #名称前缀,例:fb或者jly
groupName=${2} #分组名称,例:pro_bis
codeName=${3}  #项目名称,例:cbs
Target=${4}    #模块名称,例:core或者oms
...
fun_oms(){
    ...
    if [ $? -eq 0 ];then
        echo -e "\033[31m${time} /jenkins_workspace/${groupName}_${codeName}-oms/target/${codeName}-oms/WEB-INF/lib包含快照版api压缩包,请更换成正式版进行发布\033[0m"
        exit 1
    else
        echo -e "\033[32m${time} /jenkins_workspace/${groupName}_${codeName}-oms/target/${codeName}-oms/WEB-INF/lib无快照版api压缩包,此报错请忽略\033[0m"
    fi
}
...
fun_normal_core(){
    ...
    if [ $? -eq 0 ];then
        echo -e "\033[31m${time} /jenkins_workspace/${groupName}_${codeName}/${codeName}-core/target/${codeName}-core/lib包含快照版api压缩包,请更换成正式版进行发布\033[0m"
        exit 1
    else
        echo -e "\033[32m${time} /jenkins_workspace/${groupName}_${codeName}/${codeName}-core/target/${codeName}-core/lib无快照版api压缩包,此报错请忽略\033[0m"
    fi
}
...
fun_normal_oms(){
    ...
    if [ $? -eq 0 ];then
        echo -e "\033[31m${time} /jenkins_workspace/${groupName}_${codeName}/${codeName}-oms/target/${codeName}-oms/WEB-INF/lib包含快照版api压缩包,请更换成正式版进行发布\033[0m"
        exit 1
    else
        echo -e "\033[32m${time} /jenkins_workspace/${groupName}_${codeName}/${codeName}-oms/target/${codeName}-oms/WEB-INF/lib无快照版api压缩包,此报错请忽略\033[0m"
    fi
}
...
fun_aut_core(){
    ...
    if [ $? -eq 0 ];then
        echo -e "\033[31m${time} /jenkins_workspace/${groupName}_${codeName}/${codeName}-core/target/${codeName}-oms/WEB-INF/lib包含快照版api压缩包,请更换成正式版进行发布\033[0m"
        exit 1
    else
        echo -e "\033[32m${time} /jenkins_workspace/${groupName}_${codeName}/${codeName}-core/target/${codeName}-oms/WEB-INF/lib无快照版api压缩包,此报错请忽略\033[0m"
    fi
}
