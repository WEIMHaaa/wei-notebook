...
#一般core和oms:*-core/target/*.tar.gz,*-oms/target/*.war,**/docker_deploy.sh,**/Dockerfile
#特殊oms:target/*.war,**/docker_deploy.sh,**/Dockerfile
...
check_container_is_exist(){
  ...
  if [ "${pid}" ]; then
      echo -e "\033[32m${time} 容器名称:${comp}-${app}-${module}-${container_name} 容器id:${pid} 已存在,进行删除:docker rm -f ${comp}-${app}-${module}-${container_name}
注：可能存在假进程,如果报错该容器已存在,请前往Docker宿主机执行删除容器
...
进入容器：docker exec -it ${comp}-${app}-${module}-${container_name} /bin/bash\033[0m"
      ...
      echo -e "\033[32m${time} ${comp}-${app}-${module}-${container_name} pid不存在,防止存在假进程：docker rm -f ${comp}-${app}-${module}-${container_name},开始run,
注：可能存在假进程,如果报错该容器已存在,请前往Docker宿主机执行删除容器
删除容器：docker rm -f ${comp}-${app}-${module}-${container_name}
查看容器日志：docker logs ${comp}-${app}-${module}-${container_name}
进入容器：docker exec -it ${comp}-${app}-${module}-${container_name} /bin/bash\033[0m"
