...
#
# dev环境：9点到21点,周一到周五,每隔1小时执行一次检测disconf服务是否存在,
...
check_service_core(){
    ...
    do
        ...
        if [ "${pid}" ]; then
            echo -e "\033[32m${time} ${comp}-${app} core进程存在\033[0m";
            break;
        else
            j=$[$j+1]
            echo -e "\033[31m${time} 第${j}次检测,${comp}-${app} core进程不存在\033[0m"
        fi
        #检测3次服务仍不存在
        if [ $j -eq 3 ];then
            echo -e "\033[31m${time} ${comp}-${app} core进程不存在,即将启动项目\033[0m"
            start_core
            module=core
            ding_talk
        fi
...
check_service_oms(){
    ...
    do
        ...
        if [ "${pid}" ]; then
            echo -e "\033[32m${time} ${comp}-${app} oms进程存在\033[0m\n";
            break;
        else
            j=$[$j+1]
            echo -e "\033[31m${time} 第${j}次检测,${comp}-${app} oms进程不存在\033[0m\n"
        fi
        #检测3次服务仍不存在
        if [ $j -eq 3 ];then
            echo -e "\033[31m${time} ${comp}-${app} oms进程不存在,即将启动项目\033[0m"
            start_oms
            module=oms
            ding_talk
        fi
...
#2.1、启动core服务
start_core(){
    echo -e "\033[32m${time} ${comp}-${app} core启动中...\033[0m"
...
#2.2、启动oms服务
start_oms(){
    echo -e "\033[32m${time} ${comp}-${app} oms启动中...\033[0m"
...
ding_talk(){
    if [ $? -eq 0 ];then
        #有些系统存在假启动的情况,因此需要延迟120s检测服务是否完全正常
        sleep 120s
        if [ ${module} = core ];then
            pid=`ps aux | grep ${comp}-${app} | grep jetty | grep -v grep | awk '{print $2}'`;
...
#4、检查dubboadmin服务
check_dubboadmin(){
    #注:脚本用HOSTNAME,但是在服务器上用小写hostname
...
#注:脚本用HOSTNAME,但是在服务器上用小写hostname
    if [[ ${HOSTNAME} = "dev-zdb-nginx" ]];then
        echo -e "\033[32m${time} ${HOSTNAME} dev环境,检测disconf服务是否正常\033[0m"
    else
        echo -e "\033[32m${time} ${HOSTNAME} 非dev环境,无需检测disconf服务是否正常,退出\033[0m"
        exit 1;
    fi

    pid=`ps aux | grep fb-disconf | grep tomcat | grep -v grep | awk '{print $2}'`;
    if [ "${pid}" ]; then
        echo -e "\033[32m${time} fb-disconf进程存在\033[0m\n";
        break;
    else
...
do
    echo -e "\033[32m${time} ${comp}-${app} jly 只用tomcat启动\033[0m"
    comp=jly
    check_service_oms
done
...
do
    echo -e "\033[32m${time} ${comp}-${app} fb 只用tomcat启动\033[0m"
    comp=fb
    check_service_oms
done
