...
#2、进入mysql数据库 注：特殊字符如！需加\转义,举例：Aa\!123qwe
...
#备份数据库--设置最大缓冲区(临时性, 永久性得修改my.cnf并且重启mysql)
...
#将容器<容器ID>的/tmp/metersphere_bak.sql文件拷贝到宿主机的/tmp/wmh目录下
docker cp <容器ID>:/tmp/metersphere_bak.sql /tmp/wmh

#将宿主机/tmp/wmh/metersphere_bak.sql目录拷贝到容器<容器ID>的/tmp目录下
docker cp /tmp/wmh/metersphere_bak.sql <容器ID>:/tmp;
