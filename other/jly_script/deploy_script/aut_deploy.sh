...
4、注:单独部署core或者oms失败都会执行查看core和oms的日志,如果无法查看启动日志,报错请忽略,请直接到服务器查看\033[0m"
...
#5、拉取disconf
core_disconf(){
    echo -e "\033[32m${time} 5、从disconf拉取${codeName}的配置文件(注：aut的core和oms都用tomcat启动，aut的core和oms都只有一个配置)\033[0m"
    if [[ ${comp} = "fb" ]]; then
...
core_query_log(){
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    fi
    sleep 38s
    cd /app/${comp}-${codeName}/tomcat/logs && tail -n 200 catalina.out
    sleep 2s
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    fi
}
...
oms_query_log(){
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    fi
    sleep 38s
    cd /app/${comp}-${codeName}/tomcat/logs && tail -n 200 catalina.out
    sleep 2s
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} oms slog (注：aut的core和oms都用tomcat启动)\033[0m\n"
    fi
}
...
deploy_core(){
    ...
    echo -e "\033[32m${time} 3、停止${comp}-${codeName} core服务：cd /app/${comp}-${codeName}/tomcat/ && su ${comp}-${codeName} ${comp}-${codeName}.sh stop\033[0m"
