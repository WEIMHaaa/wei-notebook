...
#特殊情况
#bss:无需前端编译,指定jdk1.7,maven3.0,复制deploy.sh到${groupName}_${codeName},注:本脚本已指定maven和jdk版本编译,部署job无需使用默认maven和jdk构建,否则会报错
#sso,b2b-front,cb-front:无需前端编译,复制deploy.sh到${groupName}_${codeName}-oms
#aut:无需前端编译,复制aut_deploy.sh到${groupName}_${codeName}
#scheduler,mqx,etd,ecw:无需前端编译,复制deploy.sh到${groupName}_${codeName}
#core:无需前端编译,复制deploy.sh到${groupName}_${codeName}
#oms,omscore:需要前端编译,复制deploy.sh到${groupName}_${codeName}
#view:需要前端编译,复制deploy.sh到${groupName}_${codeName}(调试中)
...
#注:=左右两边不能有空格,表示赋值
#指定node版本(查看版本:node -v / nvm ls),针对从节点指定node版本,主节点无需指定,若主节点报错找不到nvm可忽略
    nvm use 6.16.0
...
if [[ ${ENV} = "请选择" ]] ; then
    ...
    echo -e "\033[31m${time} 请选择core/oms/omscore!\033[0m"
    exit 1

elif [[ ${codeName} = "bss" ]] ; then
    echo -e "\033[32m${time} 指定jdk1.7 ${codeName}\033[0m"
    #设置jdk环境变量,指定jdk1.7
    ...
    cnpm install
    echo -e "\033[32m${time} ${groupName} ${codeName} ${ENV} ${Target} 前端编译 ${npm_run_build} 注:非生产(dev、comb、test1、test2)使用同一套test1配置\033[0m"
    ${npm_run_build}
    shell_check
    echo -e "\033[32m${time} cp /jenkins_workspace/fb-script-${sh_position}-zdb/deploy.sh to /jenkins_workspace/${groupName}_${codeName}\033[0m"
    cp /jenkins_workspace/fb-script-${sh_position}-zdb/deploy.sh  /jenkins_workspace/${groupName}_${codeName}
...
#仅进行view前端编译,注:调试中
    ...
    cd /jenkins_workspace/${groupName}_${codeName}/${codeName}-view/
    cnpm install
    echo -e "\033[32m${time} ${groupName} ${codeName} ${ENV} ${Target} 前端编译 ${npm_run_build} 注:非生产(dev、comb、test1、test2使用同一套test1配置\033[0m"
    ${npm_run_build}
    shell_check
    echo -e "\033[32m${time} cp /jenkins_workspace/fb-script-${sh_position}-zdb/deploy.sh to /jenkins_workspace/${groupName}_${codeName}\033[0m"
    cp /jenkins_workspace/fb-script-${sh_position}-zdb/deploy.sh  /jenkins_workspace/${groupName}_${codeName}
